...
let rec exec stmt (locEnv: locEnv) (gloEnv: gloEnv) (store: store) : store =
    ...
    | For (e1, e2, e3, body) ->
        //返回 表达式的值 被修改的环境store1
        let (v1, store1) = eval e1 locEnv gloEnv store

        //定义 For循环辅助函数 loop
        let rec loop store0 =
            // 循环体放于环境store3 
            let store3 = exec body locEnv gloEnv store0
            let (v3, store4) = eval e3 locEnv gloEnv store3 // 执行循环后操作
            let (v4, store5) = eval e2 locEnv gloEnv store4 // 执行条件判断
            // 继续循环
            if v4 <> 0  then 
                loop (store5)
            else
                store5 //退出循环返回 环境store5
    ...
    | Switch(e,body) ->
        ...
        let rec search list =
          match list with
          | [] -> store1 // 不匹配 推出查找
          | Case(e1,body1) :: tail -> // 提取第一个case
              let (v2,store2) = eval e1 locEnv gloEnv store
              if v2 = v then exec body1 locEnv gloEnv store // 匹配执行相应操作
              else search tail // 不匹配 继续查找
        search body
...
and eval e locEnv gloEnv store : int * store =
    ...
    | Prim3 (e1, e2, e3) ->
        ...
        if v1 <> 0 then 
            ...
            let (v3,store3) = eval e3 locEnv gloEnv store1
            (v3,store3)
    // add by tianhuiwen
    | Andalso (e1, e2) ->
